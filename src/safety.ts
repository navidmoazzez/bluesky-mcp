/**
 * Decides whether a write is allowed to reach Bluesky.
 *
...
export class WriteGuard {
  ...
  check(tool: string, risk: Risk, confirm: boolean | undefined, summary: string): void {
    ...
    if (this.config.readOnly) {
      ...
      throw new WriteBlockedError(
        `${tool} is unavailable: this server is running with BLUESKY_READ_ONLY=1.`,
      );
    }
    ...
    if (risk === "destructive") {
      if (!this.config.allowDestructive) {
        ...
        throw new WriteBlockedError(
          `${tool} is unavailable: this server is running with BLUESKY_ALLOW_DESTRUCTIVE=0.`,
        );
      }
      if (confirm !== true) {
  ...
  /** Append-only record of every attempted write, when BLUESKY_AUDIT_LOG is set. */
