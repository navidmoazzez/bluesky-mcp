...
function kindOf(schema: ZodTypeAny): { kind: FlagKind; choices?: string[]; repeatable: boolean } {
  ...
  switch (typeName) {
    ...
    case "ZodArray": {
      ...
      // Treating it as JSON meant `--categories LANDSCAPES` was rejected and
      // you had to write `--categories '"LANDSCAPES"'` instead.
      const scalar =
...
export function parseArgs(argv: string[], flags: Flag[]): Record<string, unknown> {
  ...
  // One bare argument fills the first required flag, so `search-posts "cats"`
...
/**
 * `--select id,author.handle` keeps only the named fields.
 *
 * Dotted paths descend; an array is traversed element-wise so one path reaches
 * every item. This is the flag that makes a 50-post feed affordable to an
 ...
 *
 * The package ships two binaries onto the same file. Printing `bluesky-mcp` at
 * someone who typed `bluesky-cli` hands them a command that works but is not
 * the one they have in their fingers.
 */
function binName(): string {
  const name = (process.argv[1] ?? "").split("/").pop() ?? "";
  return name.startsWith("bluesky-cli") ? "bluesky-cli" : "bluesky-mcp";
}
...
export async function runCli(argv: string[]): Promise<number> {
  ...
  if (!spec) {
    ...
    emitError(
      new Error(
        hidden
          ? `${name} is unavailable: this server is running with BLUESKY_READ_ONLY=1.`
  ...
  // `--select id,author.handle` keeps only the fields asked for.
