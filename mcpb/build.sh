...
#
#   bash mcpb/build.sh    ->  mcpb/bluesky-<version>.mcpb
set -euo pipefail
...
node -e "
  const fs = require('fs');
  const p = '$root/mcpb/manifest.json';
...
npx -y @anthropic-ai/mcpb@latest pack "$build" "$root/mcpb/bluesky-$version.mcpb"

echo
echo "Built mcpb/bluesky-$version.mcpb"
